...
func getToolDescription(name string) string {
	descriptions := map[string]string{
		...
		"rule_check":             "Check if a specific rule passes for the current context. Required: action.",
		"rule_get":               "Get full content of a specific rule. Required: rule_id.",
		"rule_create_or_reuse":   "Create a new rule or reuse existing one if name+category match. Returns reused=true if exists.",
		"rule_create_from_prd":   "Generate rules from PRD requirements. Classifies non-functional requirements and creates matching rules.",
			...
			func() {
						...
						if err := json.Unmarshal(raw, &req); err != nil {
							...
							for !finished {
								...
								case <-ticker.C:
									...
									if err := encoder.Encode(progressNotif); err != nil {
										if strings.Contains(err.Error(), "broken") || strings.Contains(err.Error(), "EPIPE") || strings.Contains(err.Error(), "closed") {
											log.Printf("Connection closed during progress, exiting gracefully")
											finished = true
										}
									}
				...
				if resp != nil {
					if err := encoder.Encode(resp); err != nil {
						if strings.Contains(err.Error(), "broken") || strings.Contains(err.Error(), "EPIPE") || strings.Contains(err.Error(), "closed") {
							log.Printf("Connection closed, exiting gracefully")
							return
						}
